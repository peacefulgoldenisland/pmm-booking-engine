...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-64 bg-[var(--color-navy-900)] text-white flex flex-col h-screen sticky top-0 border-r border-white/10 shadow-luxury hidden md:flex shrink-0 z-50">
      
      {/* Brand Logo */}
      <div className="h-20 flex items-center justify-center border-b border-white/10 px-6 pt-4">
        <Link href="/admin/dashboard" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-sm bg-white/5 border border-[var(--color-gold-500)]/30 flex items-center justify-center group-hover:border-[var(--color-gold-500)] transition-colors">
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link 
              key={item.name} 
              href={item.href}
              className={`flex items-center gap-3 px-4 py-3.5 rounded-sm transition-all duration-300 group relative overflow-hidden ${
                isActive 
            ...
            >
              ...
              <span className={`text-[11px] font-bold uppercase tracking-widest ${isActive ? 'text-[var(--color-navy-900)]' : 'text-gray-300 group-hover:text-white'}`}>
                {item.name}
              </span>
      ...
      <div className="p-4 border-t border-white/10">
        <button 
          ...
          className="w-full flex items-center gap-3 px-4 py-3.5 rounded-sm text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-colors group"
        >
