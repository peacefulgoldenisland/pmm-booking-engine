    ...
    <div className="pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-serif text-[var(--color-navy-900)] flex items-center gap-3 flex-wrap">
            ...
            <AdminCardContent>
              <div className="space-y-4">
                {booking.passengersManifest?.map((pax: Passenger, idx: number) => (
                  <div key={idx} className="bg-[var(--color-surface-50)] p-5 border border-gray-100 rounded-sm flex flex-col gap-4 transition-all">
                    {editingPaxIndex === idx ? (
                           ...
                           <div>
                             ...
                             {isUploadingPaxDoc ? (
                               ...
                               <div className="flex items-center gap-3 mt-1">
                                  ...
                                  <label htmlFor={`admin-pax-upload-${idx}`} className="cursor-pointer bg-white border border-gray-300 px-3 py-2 text-xs font-bold uppercase tracking-widest text-[var(--color-navy-900)] rounded-sm hover:border-[var(--color-gold-400)] hover:text-[var(--color-gold-600)] transition-all flex items-center gap-2 shadow-sm">
                                    <UploadCloud className="w-3 h-3"/> Upload Doc
                                  </label>
                                  {editPaxData?.passportFileUrl ? (
                                    <span className="text-xs text-green-600 font-bold flex items-center gap-1"><CheckCircle2 className="w-3 h-3"/> Attached</span>
                                  ) : (
                                    <span className="text-xs text-amber-500 font-bold flex items-center gap-1"><AlertCircle className="w-3 h-3"/> Missing</span>
                                  )}
                               </div>
                             )}
                           </div>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-bold text-[var(--color-navy-900)]">{pax.fullName || 'No Name Provided'}</p>
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider">
                              {pax.nationality || 'UNKNOWN'} • {pax.gender || '?'} • Age: {pax.age || '?'} • ID: {pax.passportNumber || 'NONE'}
                            </p>
                          </div>
                        </div>
                        
                        <div className="flex items-center gap-2 shrink-0">
                          <AdminButton variant="outline" size="sm" className="px-3 h-8 text-[10px] font-bold" onClick={() => { setEditingPaxIndex(idx); setEditPaxData(pax); }}>
                            <Edit3 className="w-3 h-3 mr-1" /> Edit
                          </AdminButton>
                          
                          {pax.passportFileUrl ? (
                            <a href={pax.passportFileUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 bg-white px-3 py-1.5 border border-gray-200 rounded-sm text-[10px] font-bold text-[var(--color-navy-900)] uppercase tracking-widest hover:border-[var(--color-gold-400)] transition-all shadow-sm">
                              <FileCheck className="w-3 h-3 text-green-600" /> Doc
                            </a>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-dashed border-gray-300 rounded-sm text-[10px] font-bold text-red-400 uppercase tracking-widest">
            ...
            <AdminCardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex items-center gap-3 w-full">
                  <div className="w-10 h-10 bg-[var(--color-surface-50)] rounded-full flex items-center justify-center shrink-0"><Mail className="w-4 h-4 text-gray-500" /></div>
                ...
                <div className="flex items-center gap-3 w-full">
                  <div className="w-10 h-10 bg-[var(--color-surface-50)] rounded-full flex items-center justify-center shrink-0"><Phone className="w-4 h-4 text-gray-500" /></div>
                  <div className="flex-1 w-full">
