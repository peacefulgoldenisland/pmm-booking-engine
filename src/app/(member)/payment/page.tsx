  ...
  useEffect(() => {
    ...
    const unsubscribeDoc = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setErrorMessage("Could not find booking data.");
      }
      setIsLoading(false);
    }, (error) => {
      console.error("Error fetching booking:", error);
      setErrorMessage("Connection error. Please refresh.");
      setIsLoading(false);
    });
    ...
    return (
      <div className="min-h-screen bg-[var(--color-surface-50)] flex items-center justify-center p-4">
        <div className="bg-white p-8 md:p-12 rounded-sm border border-gray-200 shadow-luxury text-center max-w-md w-full">
          ...
          <h2 className="text-2xl font-serif text-[var(--color-navy-900)] mb-3">Booking Not Found</h2>
...
export default function PaymentPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[var(--color-surface-50)] flex flex-col items-center justify-center font-serif text-2xl text-[var(--color-navy-900)]"><Loader2 className="w-10 h-10 animate-spin text-[var(--color-gold-500)] mb-4"/> Loading Payment...</div>}>
      <PaymentContent />
    </Suspense>
  );
}